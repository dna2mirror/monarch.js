...
// resolve imports to make TypeScript compiler work
...
const NULL_STATE: IState = new NullStateImpl();
...
class EmbeddedModeData {
	...
	constructor(modeId: string, state: IState) {
		this.modeId = modeId;
		this.state = state;
	}
...
interface IMonarchTokensCollector {
	...
	nestedModeTokenize(embeddedModeLine: string, embeddedModeData: EmbeddedModeData, offsetDelta: number): IState;
}
...
class MonarchClassicTokensCollector implements IMonarchTokensCollector {
	...
	public nestedModeTokenize(embeddedModeLine: string, embeddedModeData: EmbeddedModeData, offsetDelta: number): IState {
...
class MonarchModernTokensCollector implements IMonarchTokensCollector {
	...
	constructor(modeService: IModeService, theme: any) {
...
class MonarchTokenizer implements ITokenizationSupport {

	private readonly _modeService: IModeService;
	private readonly _standaloneThemeService: any;
	private readonly _modeId: string;
	private readonly _lexer: monarchCommon.ILexer;
	private _embeddedModes: { [modeId: string]: boolean; };

	constructor(modeService: IModeService, standaloneThemeService: any, modeId: string, lexer: monarchCommon.ILexer) {
		this._modeService = modeService;
	...
	public tokenize2(line: string, lineState: IState, offsetDelta: number): TokenizationResult2 {
...
export function createTokenizationSupport(modeService: IModeService, standaloneThemeService: any, modeId: string, lexer: monarchCommon.ILexer): ITokenizationSupport {
